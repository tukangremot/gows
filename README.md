...
#### Custom Message
Allows to send custom messages to and from users. As long as you don't use the command attribute (in json payload) with the values `user-connect`, `message-send`, `group-join`, `group-leave`.
example:
```json
{
...
```
or you can send it in another format.
